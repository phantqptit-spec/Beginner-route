...
menu =  ('TỪ ĐIỂN ANH VIỆT'.center(27, '='), '1 - Tra từ điển' , '2 - Thêm từ điển' , '3 - Xóa từ điển' , '4 - Thoát chương trình')
while True:
      for u in menu:
         print(u)
      Input_number = input('Hãy ghi con số ứng với mục mà bạn muốn ở đây:')
      if Input_number == '1':
         while True:
         ...
         while True:  
            Delete_word = input('Nhập từ tiếng Anh muốn xóa hoặc nhập 0 để về menu:').lower()
            if Delete_word == '0':
               break
            elif dictionary.get(Delete_word, '') == '':
            ...
            tieptuc = True
            while tieptuc:
               choice = input('Bạn có muốn xóa thêm không? (y/n):')
               if choice != 'y' and choice != 'n': #Cấu trúc y/n y đúc đằng trên
                  print('Chỉ có thể điền y hoặc n')
               elif choice == 'y':
                  break
         ...
         print('Chỉ nhập con số ứng với từng mục')